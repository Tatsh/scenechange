...
Fixes mypy type errors and eliminates `Any` usage in the scenechange project.
...
1. Fix all mypy errors reported by `uv run mypy scenechange`
...
Place new TypedDict classes in `scenechange/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`scenechange/typing.py` when shared.
...
1. Run `uv run mypy scenechange` and capture all errors.
...
4. Run `uv run mypy scenechange` again to verify fixes.
